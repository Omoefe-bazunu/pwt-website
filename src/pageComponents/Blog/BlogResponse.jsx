...
function BlogResponse(id, category) {
  const [Comments, setComments] = useState(null)
  //  const [BlogPostReaction, setBlogPostReaction] = useState(null)
  ...
  const handleSubmit = async (event) => {
    ...
    if(!formData.email && !formData.name && !formData.email){
      try {
...
//   // FETCHING THE RECATIONS IN REAL TIME
// const FetchPostReactions = () => {
//   const q = query(collection(dbase, 'PostReactions'), where("postId", "==", id));
//   onSnapshot(q, (snapshot) => {
//     const Reactions = []; 
//     snapshot.docs.forEach((doc) => {
...
//     });
//     setBlogPostReaction(Reactions);
//     console.log(BlogPostReaction)
//   });
// }
...
//     }
  ...
  return (
    <div>
        <div className="Reactions mb-12 justify-start gap-5 pt-3 hidden">
...
<span>13</span>
          ...
          <div className="CommentsForm w-full fit mt-12 flex flex-col gap-10 ">
              ...
              <form className=' flex flex-col gap-5 justify-start my-0 mx-auto w-full' onSubmit={handleSubmit}>
                ...
                <input  type='text' placeholder='Name' name='name' required value={formData.name} onChange={handleChange} className=" outline-none py-3 px-5 border-b border-slate-300"/>
                <input type='text' placeholder='Email' name='email'  required value={formData.email} onChange={handleChange} className=" outline-none py-3 px-5 border-b border-slate-300"/>
